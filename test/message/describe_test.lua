...
local encode = require('postgres.message').encode.describe
local decode = require('postgres.message').decode.describe

function testcase.decode()
    -- test that decode Describe message for statement
    local s = concat({
        'D',
        htonl(4 + 1 + 4),
        'S',
    ...
    assert.match(msg, '^postgres%.message%.describe: ', false)
    assert.contains(msg, {
        consumed = #s,
    ...
    -- test that decode Describe message for portal
    s = concat({
        'D',
        htonl(4 + 1 + 4),
        'P',
    ...
    -- test that return error if type is not 'D'
    msg, err, again = decode(concat {
        '1',
    })
    assert.is_nil(msg)
    assert.match(err, 'invalid Describe message')
    assert.is_nil(again)
...
function testcase.encode_decode()
    -- test that encode DescribeStatement message
    ...
    -- test that encode DescribePortal message
    s = encode('portal', 'foo')
    msg, err, again = decode(s)
    assert.match(msg, '^postgres%.message%.describe: ', false)
    assert.contains(msg, {
